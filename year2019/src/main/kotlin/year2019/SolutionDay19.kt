...
class SolutionDay19 : BaseSolution() {
    ...
    override fun task1(): String {
        val tiles = discoverTiles()
        val affected = tiles.affected()
        return affected.toString()
    }

    override fun task2(): String {
        val pos = findSquareClosestToEmitter()
        val result = pos.x * 10_000 + pos.y
        return result.toString()
    }

    private fun findSquareClosestToEmitter(): Position {
        var squareBottomLeftCorner = Position(0L, 100L)
        var squareBottomLeftCornerTileType: TileType?
        while (true) {
            squareBottomLeftCornerTileType = checkTile(squareBottomLeftCorner.x, squareBottomLeftCorner.y)
            squareBottomLeftCorner = if (squareBottomLeftCornerTileType == TileType.PULLED) {
                if (isTopRightCornerInRange(squareBottomLeftCorner))
                    return Position(squareBottomLeftCorner.x, squareBottomLeftCorner.y - 99L)
                else
                    Position(0, squareBottomLeftCorner.y + 1)
            } else {
                Position(squareBottomLeftCorner.x + 1, squareBottomLeftCorner.y)
            }
        }
    }

    private fun isTopRightCornerInRange(squareBottomLeftCorner: Position): Boolean {
        val squareTopRightCorner = Position(squareBottomLeftCorner.x + 99L, squareBottomLeftCorner.y - 99L)
        return checkTile(squareTopRightCorner.x, squareTopRightCorner.y) == TileType.PULLED
    }

    private fun discoverTiles(): Tiles {
        val tiles = Tiles()
        for (x in 0L..49L)
            for (y in 0L..49L) {
                val tileType = checkTile(x, y)
                tiles.map.add(Tile(Position(x, y), tileType))
            }

        return tiles
    }

    private fun checkTile(x: Long, y: Long): TileType {
        val reply = Runner(Program(initProgramData, LinkedBlockingDeque(listOf(x, y))) { }).execute()!!
        return TileType.from(reply)
    }

    private enum class TileType(val code: Long) {
        STATIONARY(0L), PULLED(1L);
    ...
    private data class Tiles(val map: MutableSet<Tile> = mutableSetOf()) {
        fun affected() = map.filter { it.type == TileType.PULLED }.size
    }

    private data class Position(val x: Long, val y: Long)

    private data class Tile(val pos: Position, val type: TileType)
