...
class SolutionDay21 : BaseSolution() {
    ...
    override fun task1(): String {
        ...
        var prompt = ""
        var instructionSet = false
        val runner = Runner(Program(initProgramData, input) { reply ->
            if (prompt != "Input instructions:") {
                prompt += reply.toInt().toChar()
                ...
                val instructions = listOf(
                    ...
                    "NOT T J",
                    "WALK",
                ).map { instruction -> instruction.toList().map { it.code }.plus('\n'.code).map { it.toLong() } }
                instructions.forEach { input.addAll(it) }
            } else {
//                if (reply < 200)
//                    print(reply.toInt().toChar())
//                else
