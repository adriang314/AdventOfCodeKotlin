...
class SolutionDay23 : BaseSolution() {
    ...
    override fun task1(): String {
        val inputs = (0L until 50L).map { LinkedBlockingDeque(listOf(it, -1L)) }
        val computers = (0L until 50L).map {
            Computer(
                it,
                inputs[it.toInt()],
                initProgramData
            ) { sourceComp: Long, destinationComp: Long, x: Long, y: Long ->
                if (destinationComp == 255L) {
                    val total = inputs.sumOf { inp -> inp.size }
                    if (total == 0) {
                        println("[$sourceComp] sent -> [0], x=$x, y=$y")
                        inputs[0].addAll(listOf(x, y))
                    }
                } else {
                    inputs[destinationComp.toInt()].addAll(listOf(x, y))
                }
            }
        }

        runBlocking {
            computers.forEachIndexed { index: Int, computer: Computer ->
                launch(newSingleThreadContext(index.toString())) { computer.runner.execute() }
            }
        }
    ...
    private class Computer(
        private val networkAddress: Long,
        input: LinkedBlockingDeque<Long>,
        program: List<Long>,
        sendTo: (sourceComp: Long, destinationComp: Long, x: Long, y: Long) -> Unit
    ) {
        val buffer = LinkedList<Long>()

        val runner = Runner(Program(program, input) { output ->
            //println("[$networkAddress] out=$output")
            buffer.add(output)
            if (buffer.size == 3) {
                sendTo(networkAddress, buffer[0], buffer[1], buffer[2])
                buffer.clear()
            }
        })
    }
